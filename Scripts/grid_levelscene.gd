...
func change_to_scene(level_num: int):
	...
	GameManager.current_level = level_num #store current level to access it from anywhere in the game
...
func setup_level_box():
	for grid in grid_container.get_children(): #get number of grid in hbox
		for box in grid.get_children(): #get number of buttons in grid
			...
			if GameManager.data.has(str(box.get_index() + 1 + grid.get_child_count() * grid.get_index())):
				box.level_num = GameManager.data[str(box.get_index() + 1 + grid.get_child_count() * grid.get_index())]["level"]
				box.locked = false
			else:
